...
[cite_start]The core innovation lies in the use of **Microprogramming (Modul 9)** to replace rigid hardwired State Machines, allowing for a flexible control store that separates the "Decision Logic" (FSM) from the "Control Signals" (Output)[cite: 44, 45].

### 🎯 Key Capabilities
* [cite_start]**Hardware-Level Security:** PIN authentication is performed via a parallel loop accelerator, eliminating software interrupt latency[cite: 42].
* [cite_start]**Synchronous Banking Logic:** User balances are stored in Block RAM and processed via custom VHDL arithmetic functions to deduct fees in real-time[cite: 46, 47].
* [cite_start]**Automated Verification:** The system includes a self-checking testbench that autonomously validates timing constraints and logic correctness[cite: 57].
...
[cite_start]The system is designed using a **Structural VHDL** approach, integrating five distinct subsystems into a single Top-Level Entity (`atm_top.vhd`)[cite: 48, 77].
...
* **Microcode ROM (`rom_microcode.vhd`):** Acts as the look-up table for control signals. [cite_start]Instead of complex `IF-ELSE` statements in the FSM, the controller simply points to an address in the ROM to retrieve the correct LED/Display patterns[cite: 92].

### 2. The Data Plane (Muscle)
* **Auth Accelerator (`auth_accelerator.vhd`):** A dedicated hardware block that compares the input PIN against the stored user password using a `FOR LOOP` construct. [cite_start]This ensures validation happens in a single clock cycle window[cite: 80, 81].
* [cite_start]**Bank RAM (`bank_ram.vhd`):** A synchronous memory unit storing user balances[cite: 86].
    * *User 0:* Balance 9
    * *User 1:* Balance 8
* **Arithmetic Unit (`atm_pkg.vhd`):** A custom package containing the `calc_admin_fee` function. [cite_start]It automatically subtracts a fixed administration fee (1 unit) before the balance is sent to the display[cite: 88, 89].

---

## 📊 Technical Implementation Details

[cite_start]We integrated **8 Core Laboratory Modules** into this final project[cite: 164]:
...
We utilized a **Self-Checking Testbench** (`tb_atm_top.vhd`) to verify the design. [cite_start]The testbench does not require manual waveform inspection; it prints the status directly to the ModelSim console[cite: 106].

### Tested Scenarios
1.  **Security Breach Attempt:**
    * *Input:* Wrong PIN (`1111`).
    * *Expected Result:* Error LED High, State returns to IDLE.
    * [cite_start]*Status:* **PASSED**[cite: 109, 148].
...
### Prerequisites
* [cite_start]**Intel Quartus Prime (Lite/Standard)** - For Synthesis & Bitstream Generation[cite: 71].
* [cite_start]**ModelSim - Intel FPGA Edition** - For RTL Simulation[cite: 72].
...
[cite_start]This project was engineered by **Group 20** for the Digital System Design Laboratory[cite: 5, 21]:
